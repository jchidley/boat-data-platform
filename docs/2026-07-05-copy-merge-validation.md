...
The test asserted exactly **7 transitions**, **4 runtime intervals**, **2 summary rows** and **230 completed runtime seconds**. The latest disposable run measured **255 ms**, peak Node RSS **60,468 KiB**, database size **8,197,811 bytes**, and reported TimescaleDB `false`. Cleanup removed the temporary database successfully. `npm test` passed 27 Node tests with 0 failures and 1 intentional default skip; the explicit integration command passed 1 test. `cargo test --release --manifest-path tools/n2k-rust-importer/Cargo.toml` passed 5 Rust tests, `cargo fmt --check` passed, and `cargo clippy --manifest-path tools/n2k-rust-importer/Cargo.toml -- -D warnings` passed. Node syntax, JSON, shell syntax and `git diff --check` checks passed. The test-suite commit is `80de128d3409cc8948a66fe79047e5fc4f450e70`; no live migration, import or Grafana deployment was performed.
...
## First bounded live native batch

Explicit approval was given on 2026-07-21 for this one settled file. A dry run reproduced the checksum, size, 11,418 events, event-time range, four converter counts and zero skips under the staging limits. The immediate pre-batch backup is `/home/jack/boat-masterbus-prebatch-20260721T151944Z.sql`, SHA-256 `8fdc59d47d57fc31b6edd3d2e83aa29762baaa41feacdd4058b28151afecceec`.

The first COPY attempt exposed missing `DELETE` privilege on the four unlogged staging tables. Its transaction rolled back, leaving only inventory id 1 in retryable `staged` state and zero typed/staging rows. The live grant and committed `006_masterbus_v1.sql` were corrected. The next import succeeded, but verification found the live merge function was stale and had overwritten the immutable 11,418 source-event count with 8,919 distinct typed source lines. Current committed `008_masterbus_v1_merge.sql` was redeployed and an identical retry restored the source count while preserving idempotent typed counts.

Final live verification passed: inventory id 1 is `imported` with exact size `2,893,597`, checksum `846e2088af460a974e6be8e340ea84e29e3dcb75767dde11858a2952361b0347`, 11,418 source events and event range `2026-07-21T07:26:31.291Z`–`07:59:59.643Z`; typed counts are 2,937 alternator, 2,124 battery, 1,482 inverter/charger and 2,376 solar; all four stage tables are empty; all alternator rows have native source labels. Final importer elapsed time was 884 ms, peak RSS 65,776 KiB, workspace 4,076,872 bytes, live disk use 38%, and all required services and the health helper passed. Engine-history rebuild produced one open starboard interval beginning `2026-07-21T07:26:44.298Z` and no port transition.

## First Grafana typed-history dashboard

The repository dashboard was deployed on 2026-07-21 after backup `/home/jack/grafana-history-backup-20260721T153548Z`. Provisioning and dashboard file checksums matched the repository. The existing datasource had generated UID `P6F2373E0681BE28D`; a one-time provisioned delete/recreate migrated it to stable UID `boat-timescaledb`, after which the temporary deletion instruction was removed. Dashboard UID `boat-typed-history` was confirmed in Grafana 13's unified `resource` store.

Initial datasource health and direct/API SQL checks passed, but all browser panels showed a warning that no default PostgreSQL database was configured. Grafana 13 required `jsonData.database: boatdata` in addition to the already-present top-level `database: boatdata`. The live datasource and `install-pi5nvme.sh` were corrected, and `test/pi5-infra-files.test.mjs` now guards the required nested setting. After restart, the datasource API reported both database fields, connection health was OK, and the browser showed alternator, battery, transition and provenance data for the imported interval. A bounded Grafana query returned one frame with 2,870 plotted alternator samples split into port/starboard fields; the reader-role smoke query saw all 2,937 alternator rows and one transition.
