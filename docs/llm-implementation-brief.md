...
The first repository-controlled Grafana dashboard/provisioning set under `infra/pi5nvme/grafana/` was deployed on 2026-07-21. The datasource was migrated from generated UID `P6F2373E0681BE28D` to stable UID `boat-timescaledb`; dashboard UID `boat-typed-history` is present in Grafana's unified resource store. Grafana 13 also requires `jsonData.database: boatdata` for dashboard-originated PostgreSQL queries even when the top-level database field and connection health are valid; this is deployed and enforced by an installer regression test. After that fix, browser panels for alternator, battery, engine transition and acquisition provenance displayed data over the imported interval. The completed-runtime panel correctly remains empty because the only live interval is open and open intervals are deliberately excluded from completed totals. Bounded reader-role/API smoke queries returned 2,937 alternator rows, 2,870 plotted alternator samples in the selected range and one engine transition.
...
- The engine migration is deployed and rebuilt from the imported typed alternator evidence. Grafana provisioning/dashboard are deployed and verified; the deployment backup is `/home/jack/grafana-history-backup-20260721T153548Z`. Completed runtime is expected to show no data until typed stop or data-gap evidence closes an interval; never manufacture a stop from the end of the current batch.
