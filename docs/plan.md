...
Consumer-driven port order is: use the seven already gated PGNs for initial navigation history; port `129539`/`129540` only when a GNSS-quality dashboard is scheduled; defer `129285` until route/logbook reconstruction is required; and defer additional AIS/environmental PGNs until a named dashboard or analysis requires them and representative real messages are available. Decoder availability alone is not a reason to port or retain a PGN.
...
The settled-file native MasterBus staging gate is complete. A rotated hourly file was imported twice, deleted from disposable staging with its inventory, and rebuilt; counts and normalized typed content were identical. The same approved settled file was then imported into live `boatdata`, producing 2,937 alternator, 2,124 battery, 1,482 inverter/charger and 2,376 solar rows. Inventory checksum, size, 11,418 source-event lines, timestamps, native source labels, idempotence and empty staging tables were verified. A stale deployed merge function and missing staging `DELETE` privilege were corrected before final verification.
...
Migration `011_masterbus_engine_history_v1.sql` implements deterministic typed-native transition/runtime rebuilds. Its committed integration test runs against a temporary PostgreSQL database without TimescaleDB and covers exact transitions, intervals, summaries, provenance, gaps, debounce, duplicate/coalesced samples, isolation and rebuild cleanup. It was deployed live on 2026-07-21 after schema backup `/home/jack/boat-engine-schema-backup-20260721T114643Z.sql`. Rebuild from the first typed batch produced one open starboard interval beginning at `2026-07-21T07:26:44.298Z` UTC and no port transition, agreeing with the starboard-only observation. The repository-controlled bounded Grafana dashboard was deployed on 2026-07-21 with stable datasource UID `boat-timescaledb`. Grafana 13 required both the top-level database and `jsonData.database: boatdata`; without the latter, connection health passed but browser panels reported no default database. The installer and regression test now enforce both fields. Provisioning, checksums, API health, bounded reader-role/API queries and browser display were verified. Completed runtime intentionally has no row because the only starboard interval is open. Logbook integration remains deliberately deferred until the two remaining physical engine combinations and typed runtime semantics are trustworthy.
