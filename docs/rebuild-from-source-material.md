...
The native MasterBus rebuild procedure is now validated on a settled hourly file: convert, merge, duplicate merge, delete typed rows and inventory, then re-import. The normalized typed exports were identical after rebuild and disposable staging tables were empty after merge. Use `011_masterbus_engine_history_v1.sql` after native typed replay to deterministically rebuild engine transitions/runtime; it uses native alternator evidence, preserves data-gap/open-interval semantics, and carries raw file/line provenance through interval endpoints. The committed `npm run test:engine-history:integration` regression independently proves the derived delete/rebuild path in a temporary PostgreSQL database without TimescaleDB.
...
Do not use the active hourly MasterBus file, live Signal K output, or live `pi5nvme` for broad rebuild validation. The engine migration, first explicitly approved settled native batch and Grafana dashboard were deployed live and verified on 2026-07-21; additional typed imports remain explicit approval/prerequisite gates. When restoring Grafana 13 provisioning, preserve stable UID `boat-timescaledb` and set `database: boatdata` both at datasource top level and under `jsonData`.
